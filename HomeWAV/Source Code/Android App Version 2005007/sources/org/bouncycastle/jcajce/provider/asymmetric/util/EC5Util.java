...
public class EC5Util {
    ...
    public static ECCurve getCurve(ProviderConfiguration providerConfiguration, X962Parameters x962Parameters) {
        ...
        if (x962Parameters.isNamedCurve()) {
            instance = ECUtil.getNamedCurveByOid(ASN1ObjectIdentifier.getInstance(x962Parameters.getParameters()));
        } else if (x962Parameters.isImplicitlyCA()) {
            return providerConfiguration.getEcImplicitlyCa().getCurve();
        } else {
            instance = X9ECParameters.getInstance(x962Parameters.getParameters());
        }
