package com.forasoft.homewavvisitor.ui.fragment.conversations;

import android.view.View;
import android.widget.Button;
import com.forasoft.homewavvisitor.R;
import com.forasoft.homewavvisitor.extension.CommonKt;
import com.forasoft.homewavvisitor.presentation.presenter.conversations.VideoRecordPresenter;
import com.forasoft.homewavvisitor.ui.fragment.conversations.CameraView;
import java.io.File;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function1;
import kotlin.jvm.internal.Intrinsics;
import kotlin.jvm.internal.Lambda;

@Metadata(bv = {1, 0, 3}, d1 = {"\u0000\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\u0010\u0000\u001a\u00020\u00012\b\u0010\u0002\u001a\u0004\u0018\u00010\u0003H\n¢\u0006\u0002\b\u0004"}, d2 = {"<anonymous>", "", "it", "Landroid/view/View;", "invoke"}, k = 3, mv = {1, 1, 16})
/* compiled from: VideoRecordFragment.kt */
final class VideoRecordFragment$onViewCreated$4 extends Lambda implements Function1<View, Unit> {
    final /* synthetic */ VideoRecordFragment this$0;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    VideoRecordFragment$onViewCreated$4(VideoRecordFragment videoRecordFragment) {
        super(1);
        this.this$0 = videoRecordFragment;
    }

    public /* bridge */ /* synthetic */ Object invoke(Object obj) {
        invoke((View) obj);
        return Unit.INSTANCE;
    }

    public final void invoke(View view) {
        CameraView access$getCamera$p = this.this$0.camera;
        if (access$getCamera$p != null) {
            access$getCamera$p.takePicture(new CameraView.PictureCallback(this) {
                final /* synthetic */ VideoRecordFragment$onViewCreated$4 this$0;

                {
                    this.this$0 = r1;
                }

                public final void onPictureAvailable(File file) {
                    VideoRecordPresenter presenter = this.this$0.this$0.getPresenter();
                    Intrinsics.checkExpressionValueIsNotNull(file, "it");
                    presenter.onPictureTaken(file);
                }
            });
        }
        CommonKt.hide((Button) this.this$0._$_findCachedViewById(R.id.btn_take_photo));
        CommonKt.hide((Button) this.this$0._$_findCachedViewById(R.id.btn_record));
        CommonKt.hide((Button) this.this$0._$_findCachedViewById(R.id.btn_rotate));
    }
}
