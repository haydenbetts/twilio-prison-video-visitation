...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    private void decodeInitializationData(List<byte[]> list) {
        ...
        if (list != null && list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            this.defaultColorRgba = ((bArr[26] & UByte.MAX_VALUE) << 24) | ((bArr[27] & UByte.MAX_VALUE) << Tnaf.POW_2_WIDTH) | ((bArr[28] & UByte.MAX_VALUE) << 8) | (bArr[29] & UByte.MAX_VALUE);
            ...
            if (z) {
                float f = ((float) ((bArr[11] & UByte.MAX_VALUE) | ((bArr[10] & UByte.MAX_VALUE) << 8))) / ((float) i);
