    ...
    public void queueInput(ByteBuffer byteBuffer) {
        ...
        if (z) {
            while (position < limit) {
                writePcm32BitFloat((byteBuffer.get(position) & UByte.MAX_VALUE) | ((byteBuffer.get(position + 1) & UByte.MAX_VALUE) << 8) | ((byteBuffer.get(position + 2) & UByte.MAX_VALUE) << Tnaf.POW_2_WIDTH) | ((byteBuffer.get(position + 3) & UByte.MAX_VALUE) << 24), this.buffer);
                position += 4;
            }
        } else {
            while (position < limit) {
                writePcm32BitFloat(((byteBuffer.get(position) & UByte.MAX_VALUE) << 8) | ((byteBuffer.get(position + 1) & UByte.MAX_VALUE) << Tnaf.POW_2_WIDTH) | ((byteBuffer.get(position + 2) & UByte.MAX_VALUE) << 24), this.buffer);
                position += 3;
            }
        }
