    ...
    private VideoBuffer packetize(DataBuffer dataBuffer, int i, int i2) {
        ...
        if (ceil == 1) {
            ...
            while (i3 < ceil) {
                ...
                DataBuffer allocate = DataBuffer.allocate(getHeaderPadding() + i5);
                allocate.resize(i5, getHeaderPadding());
                allocate.write(dataBuffer.subset(i4, i5), 0);
                arrayList.add(new Packet(allocate, i3 == 0));
                i4 += i5;
                i3++;
            }
        }
